...
/** Proves that the client received the server's accepted-tracking challenge. */
public record ReturnCapsuleTrackingReadyPacket(UUID ticketId, UUID capsuleId)
        implements ICustomPacketPayload {

    public ReturnCapsuleTrackingReadyPacket(FriendlyByteBuf buffer) {
    ...
    public void handle(Supplier<NetworkEvent.Context> context) {
        ...
        if (player != null) ReturnCapsuleService.confirmClientReady(player, ticketId, capsuleId);
