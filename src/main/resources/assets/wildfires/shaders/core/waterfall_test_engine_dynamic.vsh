...
void main() {
    ...
    vec3 recalculatedNormal = safeNormalize(Normal + deriv * PlumeDirection);
    vec3 localPosition = displaced * ModelScale;
    vec4 viewPosition = ModelViewMat * vec4(localPosition, 1.0);
    texCoord0 = UV0;
    vertexColor = Color;
    viewDirection = safeNormalize(-viewPosition.xyz);
    viewNormal = safeNormalize(mat3(ModelViewMat)
            * safeNormalize(recalculatedNormal / ModelScale));
    plumeDirection = safeNormalize(mat3(ModelViewMat) * (PlumeDirection * ModelScale));
    plumePos = arg;
